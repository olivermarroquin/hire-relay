...
export function CandidateListWithFilters({
  candidates,
  roleId,
  roleTitle,
}: CandidateListWithFiltersProps) {
  const [activeTab, setActiveTab] = useState<FilterTab>("all");

  // Apply role filter first (server-determined, not interactive), then status tab filter.
  const roleFiltered = roleId
    ? candidates.filter((c) => c.role_id === roleId)
    : candidates;
  const filtered =
    activeTab === "all"
      ? roleFiltered
      : roleFiltered.filter((c) => c.status === activeTab);

  const countFor = (tab: FilterTab) =>
    tab === "all"
      ? roleFiltered.length
      : roleFiltered.filter((c) => c.status === tab).length;
